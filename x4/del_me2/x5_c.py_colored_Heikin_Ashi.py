def t (g, strike1a, strike2a, perda, intervla):
    ...
    # https://stackoverflow.com/questions/56170609/how-to-add-colors-to-printed-text
...
###################################################
    data_y = yf.download(g, period='5d', interval='1d',prepost = True)
    df_y=pd.DataFrame(data_y)
    p=df_y['Close'].shift(1)
    pp=p[4].round(2)
    print('\n\n','yesterday close: ',pp,'\n\n')
...
#https://www.investopedia.com/trading/heikin-ashi-better-candlestick/
    ...
    for x in df.index:
        ...
        df['a_High']=max(df['High'].loc[x],df['Open'].loc[x],df['Close'].loc[x])
        df['a_Low']=min(df['Low'].loc[x],df['Open'].loc[x],df['Close'].loc[x])
...
#    sys.exit()
    ...
    if 'd' in perda:
        df['x']=df3['Datetime'].dt.time
        df['d']=df3['Datetime'].dt.day_name()
        df['u']=df3['Datetime'].dt.date
    else:
        df['x']='-'
        df['d']=df3['Date'].dt.day_name()
        df['u']=df3['Date'].dt.date
...
#    df3['price_delta']=df3['Close']-pp

    df3['p_Vol']=df3['Volume'].shift(1)
    df3['vol_delta']=-1*(df3['p_Vol']-df3['Volume'])
    df3['vol_delta']=df3['vol_delta'].round(3)
    
    df3['vol_3dy']=df3['Volume'].shift(3)
    df['vol_3dy'] = data['Volume'].rolling(3).mean()
    df3['vol_5dy']=df3['Volume'].shift(5)
    df['vol_5dy'] = data['Volume'].rolling(5).mean()
...
#    df3=df3[['x','d','u','Ticker', 'price_delta','Close', 'Volume','green','greenby','strike1', 'strike2', 'Adj Close', 'Datetime', 'High'        , 'Low', 'Open', '(from_strike1)', 'p_Price', 'p_Vol', 'vol_delta']]
    ...
    df3=df3[['x','d','u','Ticker', 'price_delta','Close', 'Volume','green','greenby','strike1','vol_3dy','vol_5dy' ]]

#    df3=df3[['d','u','x','Ticker','Price','Vol','green', 'greenby','price_delta', 'p_Vol', 'vol_delta','Low','High','Close']]
#    print('mmmm')


    if '1m' in intervl: # evert minute 

#        print("\033[1;32;40m Bright Green \n")
        for x in df3.index:
                df3['Volume'].loc[x]=numerize.numerize(np.float32(df3['Volume'].loc[x]).item())
                if df3['vol_3dy'].loc[x] > 0:
                    df3['vol_3dy'].loc[x]=numerize.numerize(np.float32(df3['vol_3dy'].loc[x]).item())
                if df3['vol_5dy'].loc[x]>0:
                    df3['vol_5dy'].loc[x]=numerize.numerize(np.float32(df3['vol_5dy'].loc[x]).item())
    #    print(df3.tail(160))
    ...
    elif '60m' in intervl:    # every hour
        for x in df3.index:
                df3['Volume'].loc[x]=numerize.numerize(np.float32(df3['Volume'].loc[x]).item())
                if df3['vol_3dy'].loc[x] > 0:
                    df3['vol_3dy'].loc[x]=numerize.numerize(np.float32(df3['vol_3dy'].loc[x]).item())
